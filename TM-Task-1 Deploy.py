# ASSIGNMENT
# TEXT MINING (NLP) - DEPLOYMENT CODE
# RESHMA SEN N
...
def generate_wordcloud(text, title):
    if not text.strip():
        st.warning(f"No valid words found for {title}. Skipping word cloud.")
        return
...
def generate_ngram_wordcloud(text, n, title):
    ...
    if not tokens:
        st.warning(f"No valid n-grams found for {title}. Skipping word cloud.")
        return
